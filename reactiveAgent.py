...
class reactiveAgent(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, x, y, company, surface, name):
        ...
        self.dx = 0
        self.dy = 0
        self.direction = constants.ROT[3] # initial position pointing left
    ...
    def agentDecision(self):
        ...
        if self.isLowBattery():
            ...
            # if the agent stoped to recharge
    ...
    def pickUpDelivery(self):
        ...
        entity = world.World.getWorldObject(world.World, (self.x + self.dx),
                                            (self.y + self.dy)).__dict__  # get entity in front of agent
        id = entity.get(
            'info')  # get id of delivery that this entity has. if it has a delivery, the 'info' attribute has its id.
        ...
        print(" [{}] has delivery: {}. Pp: ({}, {}), Dp:({}, {})".format(self.name, deli.get('id'), deli.get('x'),
                                                                         deli.get('y'), deli.get('dp_x'),
                                                                         deli.get('dp_y')))

        self.updateAgent(deli.get('id'))  # update agent, so it knows it has a cargo and the id

        world.World.updateDelivery(world.World, deli.get('id'), 'agent',
                                   self.name + '-' + self.myCompany)  # update delivery, so it knows which agents is delivering
        world.World.updateBuilding(world.World, deli.get('x'), deli.get(
            'y'))  # update building (remove delivery from it) -> set to false in objects, dont remove from deliveries list
    ...
    def hasObstacle(self):
        entity = world.World.getWorldObject(world.World, (self.x + self.dx), (self.y + self.dy)).__dict__
        return entity.get('type') == 'cell' and entity.get('obs')
    ...
    def isDeliveryPoint(self):
        ...
        return self.x+self.dx == entity.get('dp_x') and self.y+self.dy == entity.get('dp_y')
    ...
    def isHeadQuarters(self):
        entity = world.World.getWorldObject(world.World, (self.x + self.dx), (self.y + self.dy)).__dict__
        return entity.get('type') == self.myCompany
